/*
 * DEVELOPMENT ONLY. This script is intentionally restricted to the fake user
 * below so it cannot be used to load, modify, or expose real health data.
 */
...
async function seed() {
    ...
    await HealthLog.insertMany([
        {
            userId: SAMPLE_USER_ID, date: daysAgo(2),
            ...
            sleep: { hours: 7.5, quality: 4 }, stress: { level: 3, note: "Example low-stress day" }
        },
        {
            userId: SAMPLE_USER_ID, date: daysAgo(1),
            ...
            sleep: { hours: 7, quality: 3 }, stress: { level: 4, note: "Example busy day" }
        }
    ]);

    await Habit.insertMany([
        {
            userId: SAMPLE_USER_ID, name: "Example morning walk", category: "activity", targetDays: 66,
            currentStreak: 3, longestStreak: 5, lastCompletedDate: daysAgo(0), freezeCredits: 1,
            checkIns: [{ date: daysAgo(3) }, { date: daysAgo(1) }, { date: daysAgo(0) }],
            freezeDays: [{ date: daysAgo(2), reason: "Example development freeze" }]
        },
        {
            userId: SAMPLE_USER_ID, name: "Example hydration reminder", category: "nutrition", targetDays: 66,
            currentStreak: 1, longestStreak: 1, lastCompletedDate: daysAgo(0), freezeCredits: 2,
